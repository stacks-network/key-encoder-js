...
KeyEncoder.prototype.privatePEMToDER = function(privateKeyPEM) {
    var privateKeyObject = ECPrivateKeyASN.decode(privateKeyPEM, 'pem', this.privatePEMOptions)
    return ECPrivateKeyASN.encode(privateKeyObject, 'der', this.privatePEMOptions)
}

KeyEncoder.prototype.privateDERToHex = function(privateKeyDER) {
    var privateKeyObject = ECPrivateKeyASN.decode(privateKeyDER, 'der')
    return privateKeyObject.privateKey.toString('hex')
}

KeyEncoder.prototype.privateDERToPEM = function(privateKeyDER) {
    var privateKeyObject = ECPrivateKeyASN.decode(privateKeyDER, 'der')
    return ECPrivateKeyASN.encode(privateKeyObject, 'pem', this.privatePEMOptions)
}
...
/*if (originalFormat === 'hex' && destinationFormat === 'pem') {
    var privateKeyObject = secp256k1.keyFromPrivate(privateKey, 'hex'),
        publicKeyHex = privateKeyObject.getPublic('hex')
    return this.hexToPrivatePEM(privateKey, publicKeyHex)
} else if (originalFormat === 'hex' && destinationFormat === 'der') {
    var privateKeyObject = secp256k1.keyFromPrivate(privateKey, 'hex'),
        publicKeyHex = privateKeyObject.getPublic('hex')
    return this.hexToPrivateDER(privateKey)
} else if (originalFormat === 'pem' && destinationFormat === 'hex') {
    return this.privatePEMToHex(privateKey)
} else if (originalFormat === 'der' && destinationFormat === 'hex') {
    return this.privateDERToHex(privateKey)
} else if (originalFormat === 'pem' && destinationFormat === 'der') {
    return this.privatePEMToDER(privateKey)
} else if (originalFormat === 'der' && destinationFormat === 'pem') {
    return this.privateDERToPEM(privateKey)
}*/


/*if (originalFormat === 'hex' && destinationFormat === 'pem') {
    return this.hexToPublicPEM(publicKey)
} else if (originalFormat === 'hex' && destinationFormat === 'der') {
    return this.hexToPublicDER(publicKey)
} else if (originalFormat === 'pem' && destinationFormat === 'hex') {
    return this.publicPEMToHex(publicKey)
} else if (originalFormat === 'der' && destinationFormat === 'hex') {
    return this.publicDERToHex(publicKey)
} else if (originalFormat === 'pem' && destinationFormat === 'der') {
    throw 'cannot convert pem to der'
} else if (originalFormat === 'der' && destinationFormat === 'pem') {
    throw 'cannot convert der to pem'
}*/
